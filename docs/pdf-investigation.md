...
Tracing does not recover semantic strokes, editable handwriting, original Freeform objects, gradients, or subpixel source geometry. Pixels at supported transparency levels become opaque quantized SVG fills. Components whose ink lies mostly below the vector alpha floor stay raster, so they soften rather than sharpen when zoomed. Smooth-traced thin strokes are weight-true approximations of the source samples; they cannot recover detail finer than a source pixel. Low-alpha content remains raster; only positively identified highlighter strokes replace nonzero alpha with `255`. Opaque content remains raster rather than being guessed into vectors.
