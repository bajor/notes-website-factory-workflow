...
Tracing does not recover semantic strokes, editable handwriting, original Freeform objects, gradients, or subpixel source geometry. Pixels at supported transparency levels become opaque quantized SVG fills. Untraceable marks that are wide, faint, or multicolor stay raster, so they soften rather than sharpen when zoomed. Reconstructed sub-pixel strokes are crisp but assume opaque ink; writing only two or three source pixels tall stays hard to read however it is drawn. Smooth-traced thin strokes are weight-true approximations of the source samples; they cannot recover detail finer than a source pixel. Low-alpha content remains raster; only positively identified highlighter strokes replace nonzero alpha with `255`. Opaque content remains raster rather than being guessed into vectors.
